...
export_single_group() {
    ...
    for img in "${img_raw_list[@]}"; do
        ...
        local short_name="${DIGEST_TO_SHORTNAME[$img]:-unknown}"
        local save_ref
        ...
        echo "$img $save_ref $short_name" >> "${TAG_MAP_FILE}.${group_name}"
        echo "保存完成：$tar_path (tag: $save_ref)"
    done

    # 合并分组映射到全局映射文件
    cat "${TAG_MAP_FILE}.${group_name}" >> "${TAG_MAP_FILE}"
